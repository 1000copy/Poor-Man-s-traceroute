...
class Ping:
    ...
    def do_one(self,dest_addr, timeout,ttl): 
        ...
        delay = self.echo_reply_decode( my_ID, timeout)      
        self.my_socket.close() 
        return delay 
     
    def ping(self,dest_addr, timeout = 2, count = 4): 
        print "ping %s...\n" % dest_addr, 
        for i in xrange(count):         
            try: 
                delay  =  self.do_one(dest_addr, timeout,i) 
            ...
            if delay  ==  None: 
                ...
                delay  =  delay * 1000 
                print "\tin %0.4fms" % delay  
import unittest     
class tt(unittest.TestCase):
    def setUp(self):
        self.ping= Ping()
    def test_sample(self):
        self.ping.ping("163.com") 
