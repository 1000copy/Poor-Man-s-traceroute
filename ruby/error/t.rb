...
icmp_sockaddr = Socket.pack_sockaddr_in( localport, "127.0.0.1" )
icmp_sock.bind( icmp_sockaddr )

begin
        icmp_sock.connect( Socket.pack_sockaddr_in( 999, ARGV[0] ) )
rescue SocketError => err_msg
        puts "Can't connect to remote host (#{err_msg})." ; exit 1
end

until ttl == max_ttl
        icmp_sock.setsockopt( 0, Socket::IP_TTL, ttl )
        icmp_sock.send( "RubyRoute says hello!", 0 )
